...
impl BookMeta {
	fn create<R: io::Seek + io::Read>(
		...
		let spine = {
			let mut builder = NodeTreeBuilder::create()?;
			let mut items = Vec::new();
			for item in spine {
				let res = resources
					.get(&item.idref)
		...
		let total_words = spine.iter().map(|s| s.words).sum();
...
pub fn spawn_illustrator(records: RecordKeeper, id: library::BookId) -> Illustrator {
	...
	let handle = std::thread::spawn(move || {
		...
		records.record_book_state(id, book_meta.total_words, None)?;
